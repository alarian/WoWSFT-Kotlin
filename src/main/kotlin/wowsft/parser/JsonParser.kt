...
open class JsonParser {
    ...
    @Autowired private lateinit var shipsList: LinkedHashMap<String, LinkedHashMap<String, LinkedHashMap<String, LinkedHashMap<Int, MutableList<ShipIndex>>>>>
                                ...
                                if (ship.research) {
                                    ...
                                    if (shipsList[nation.key]?.get(realShipType.key)?.get(shipType)?.get(cTier - 1)?.isNotEmpty()!!) {
                                        shipsList[nation.key]?.get(realShipType.key)?.get(shipType)?.get(cTier - 1)?.forEach { tShip ->
                                            if (ships[tShip.index]?.typeinfo?.species.equals(shipType, true)) {
